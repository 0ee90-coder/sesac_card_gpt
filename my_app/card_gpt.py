...
if "messages" not in st.session_state:
    st.session_state["messages"] = [
        {"role": "assistant", "content": "안녕하세요, 저는 카드 추천 AI Assistant 입니다."} # 첫화면에 아무것도 안띄우고 싶으면 이부분을 작성 안하면 됨
    ]
...
# 프롬프트 템플릿 작성: 대화 기록을 기반으로 ai가 응답을 유도
system_prompt = """
...
def get_user_input(question):
    return {
        "chat_history": st.session_state["pre_memory"].chat_memory.messages,    # session state에 
        "question": question,
